...
// Make home position a variable that can be modified in the EEPROM.
float manual_z_home_pos;
...
static void clean_up_after_endstop_or_probe_move() {
 ...
 // TRIGGERED_WHEN_STOWED_TEST can easily be extended to servo probes, ... if needed.
  ...
  static bool set_probe_deployed(bool deploy) {
    ...
    //AW do_blocking_move_to(oldXpos, oldYpos, current_position[Z_AXIS]); // return to position before deploy
   ...
   */
    ...
    if (mbl.active()) {
      ...
      if (axis_homed[X_AXIS] && axis_homed[Y_AXIS] && axis_homed[Z_AXIS]) {
		...
		//AWDEBUG
		SERIAL_ECHOLNPAIR("AWDEBUG| already homed, pre_home_z = ", pre_home_z);
      }
      else {
        mbl.set_active(false);
        current_position[Z_AXIS] = pre_home_z;
		//AWDEBUG
		SERIAL_ECHOLNPAIR("AWDEBUG| pre_home_z = ", pre_home_z);
      }
      #if ENABLED(DEBUG_LEVELING_FEATURE)
...
void ok_to_send() {
  ...
  float delta_safe_distance_from_top() {
    ...
    inverse_kinematics(cartesian);
    return abs((distance - delta[A_AXIS])/2);
  }
